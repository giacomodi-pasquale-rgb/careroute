...
if(enrichmentBatch){
  ...
  document.getElementById('enrichmentLead').textContent=`Fragmentation-priority batch ${enrichmentBatch.batchId} · ${enrichmentBatch.providerSystem} · reviewed ${new Date(`${enrichmentBatch.reviewedAt}T12:00:00`).toLocaleDateString()}. ${enrichmentBatch.selectionMethod}`;
  document.getElementById('enrichmentSummary').innerHTML=[['Locations reviewed',summary.reviewed],['Evidence checks',summary.claimsChecked],['Claims resolved',summary.claimsResolved],['Cleared public-evidence gates',summary.releaseEligible]].map(([label,value])=>`<article><strong>${value}</strong><span>${escapeHtml(label)}</span></article>`).join('');
  document.getElementById('enrichmentRecords').innerHTML=enrichmentBatch.records.map(record=>{
    ...
    const priority=record.fragmentationPriority?.signals?.length ? `<div class="priority-reasons"><strong>Why prioritized</strong>${record.fragmentationPriority.signals.map(signal=>`<span>${escapeHtml(signal)}</span>`).join('')}</div>`:'';
    const eligible=record.status==='release-eligible';
    return `<article class="enrichment-card"><header><div><span class="ccn">${record.cmsCertificationNumber?`CMS ${escapeHtml(record.cmsCertificationNumber)}`:'Provider location · CMS match unresolved'}</span><h3>${escapeHtml(record.name)}</h3><p>${escapeHtml(record.location)}</p></div><span class="review-status ${eligible?'verified':'pending'}">${eligible?'Public evidence complete':'Held safely'}</span></header>${priority}<div class="confidence-row"><strong>${resolved} of 8 domains resolved</strong><i><em style="width:${resolved/8*100}%"></em></i></div><ul>${domains}</ul><p class="hold-reason"><strong>Release decision:</strong> ${escapeHtml(record.statusReason)}</p><a href="${escapeHtml(record.sourceUrl)}" target="_blank" rel="noopener">Inspect provider source ↗</a></article>`;
  }).join('');
